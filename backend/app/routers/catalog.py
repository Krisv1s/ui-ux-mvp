...
def get_products(
        ...
        product_type: Optional[List[int]] = Query(None, description="Фильтр по типу продукта (массив type_id)"),
        db: Session = Depends(get_db)
):
    query = db.query(
        models.Product.id,
        ...
        models.Product.id == models.ProductSupplier.product_id, isouter=True
    ).group_by(
        models.Product.id
    )

    if product_type:
        query = query.filter(models.Product.type.in_(product_type))
